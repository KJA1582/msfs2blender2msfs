...
def array_to_accessor(
    ...
    create_buffer_view=True,
    emulate_asobo_optimization=False,
):
    if (
        emulate_asobo_optimization
    ):  # Asobo uses different data types for some components
...
def __gather_normal(blender_primitive, export_settings):
    ...
    if export_settings["emulate_asobo_optimization"]:
        component_type = (
            gltf2_io_constants.ComponentType.Byte
        )  # Asobo uses bytes instead of floats
        data_type = (
            gltf2_io_constants.DataType.Vec4
        )  # Asobo uses a VEC4 instead of a VEC3 for normals
        normal *= 127
        normal = normal.astype(
            gltf2_io_constants.ComponentType.to_numpy_dtype_asobo(component_type)
        )  # Convert the numpy array data type to bytes
    return {
...
def __gather_tangent(blender_primitive, export_settings):
    if (
        not export_settings[gltf2_blender_export_keys.TANGENTS]
        and not export_settings["emulate_asobo_optimization"]
    ):  # Force export tangents for optimized meshes
        return {}
...
def __gather_texcoord(blender_primitive, export_settings):
    ...
    if export_settings[gltf2_blender_export_keys.TEX_COORDS]:
        ...
        while blender_primitive["attributes"].get(tex_coord_id) is not None:
            ...
            if export_settings["emulate_asobo_optimization"]:
                component_type = (
                    gltf2_io_constants.ComponentType.Short
                )  # Asobo uses shorts instead of floats
                tex_coord = tex_coord.astype(
                    gltf2_io_constants.ComponentType.to_numpy_dtype_asobo(
                        component_type
                    )
                )  # Convert the numpy array data type to shorts
...
def __gather_colors(
    blender_primitive, export_settings
):  # TODO: I don't think this is right for optimization, the values don't match the same as the ones that were built by the sim
    attributes = {}
        ...
        while blender_primitive["attributes"].get(color_id) is not None:
            ...
            if export_settings["emulate_asobo_optimization"]:
                if blender_primitive["VertexType"] == "VTX":
                    ...
                    )  # Unskinned meshes use unsigned shorts
                else:
                    component_type = (
                        gltf2_io_constants.ComponentType.Byte
                    )  # Skinned meshes use bytes
                    colors[:] = -1
                colors = colors.astype(
                    gltf2_io_constants.ComponentType.to_numpy_dtype_asobo(
                        component_type
                    )
                )  # Convert the numpy array data type
            else:
...
def __gather_skins(blender_primitive, export_settings):
    ...
    if export_settings[gltf2_blender_export_keys.SKINS]:
        ...
        while blender_primitive["attributes"].get(joint_id) and blender_primitive[
            "attributes"
        ].get(weight_id):
            if bone_set_index >= 1:
            ...
            if (
                not export_settings["gltf_all_vertex_influences"]
                and not vertex_type == "BLEND1"
            ):
            ...
            if export_settings["emulate_asobo_optimization"]:
                if blender_primitive["VertexType"] == "BLEND1":
                    ...
                    )  # BLEND1 primitives use scalar instead of VEC4
                else:
                    weight_component_type = (
                        gltf2_io_constants.ComponentType.UnsignedShort
                    )  # BLEND4 primitives use unsigned shorts instead of floats
