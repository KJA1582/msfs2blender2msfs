...
from io_scene_gltf2_msfs_kh_2024.io.com.gltf2_io_extensions import Extension
from io_scene_gltf2_msfs_kh_2024.blender.exp import (
    gltf2_blender_gather_texture_info,
    gltf2_blender_export_keys,
)
from io_scene_gltf2_msfs_kh_2024.blender.exp import gltf2_blender_search_node_tree

from io_scene_gltf2_msfs_kh_2024.blender.exp import (
    gltf2_blender_gather_materials_pbr_metallic_roughness,
)
from io_scene_gltf2_msfs_kh_2024.blender.exp import gltf2_blender_gather_materials_unlit
...
def gather_material(blender_material, export_settings):
    ...
    material = gltf2_io.Material(
        ...
        occlusion_texture=__gather_occlusion_texture(
    ...
    export_user_extensions(
        "gather_material_hook", export_settings, material, blender_material
    )
...
def __gather_double_sided(blender_material, export_settings):
    ...
    old_double_sided_socket = gltf2_blender_get.get_socket_old(
        blender_material, "DoubleSided"
    )
    if (
        old_double_sided_socket is not None
        and not old_double_sided_socket.is_linked
        and old_double_sided_socket.default_value > 0.5
    ):
        return True
    return None
...
def __gather_extensions(blender_material, export_settings):
    ...
    clearcoat_extension = __gather_clearcoat_extension(
...
def __gather_occlusion_texture(blender_material, orm_texture, export_settings):
    ...
    if occlusion is None:
        occlusion = gltf2_blender_get.get_socket_old(blender_material, "Occlusion")
    return (
        gltf2_blender_gather_texture_info.gather_material_occlusion_texture_info_class(
...
def __gather_clearcoat_extension(blender_material, export_settings):
    ...
    if (
        isinstance(clearcoat_socket, bpy.types.NodeSocket)
        and not clearcoat_socket.is_linked
    ):
...
def __gather_transmission_extension(blender_material, export_settings):
    ...
    if (
        isinstance(transmission_socket, bpy.types.NodeSocket)
        and not transmission_socket.is_linked
    ):
...
def __gather_material_unlit(blender_material, export_settings):
    ...
    material = gltf2_io.Material(
        ...
        double_sided=__gather_double_sided(blender_material, export_settings),
        extensions={
            "KHR_materials_unlit": Extension("KHR_materials_unlit", {}, required=False)
        },
        extras=__gather_extras(blender_material, export_settings),
        name=__gather_name(blender_material, export_settings),
    ...
    export_user_extensions(
        "gather_material_unlit_hook", export_settings, material, blender_material
    )
