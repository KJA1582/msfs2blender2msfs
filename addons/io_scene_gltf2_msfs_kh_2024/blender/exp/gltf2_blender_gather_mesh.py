...
from io_scene_gltf2_msfs_kh_2024.blender.exp import gltf2_blender_gather_primitives
...
def gather_mesh(
    ...
    if not skip_filter and not __filter_mesh(
        blender_mesh, library, vertex_groups, modifiers, export_settings
    ):
        return None
    ...
    if len(mesh.primitives) == 0:
        print_console(
            "WARNING",
            "Mesh '{}' has no primitives and will be omitted.".format(mesh.name),
        )
        return None
...
def __gather_primitives(
    ...
    return gltf2_blender_gather_primitives.gather_primitives(
        blender_mesh,
        library,
        blender_object,
        vertex_groups,
