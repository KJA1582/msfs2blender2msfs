...
def gather_animations(
    ...
    offset: int,
    export_settings,
) -> typing.Tuple[typing.List[gltf2_io.Animation], typing.Dict[str, typing.List[int]]]:
    """
    ...
    for blender_action, track_name, on_type in blender_actions:
        ...
        if on_type == "OBJECT":  # Not for shapekeys!
            if blender_object.animation_data.action is None or (
                blender_object.animation_data.action.name != blender_action.name
            ):
...
def __gather_animation(
    ...
    try:
        ...
        print_console(
            "WARNING",
            "Animation '{}' could not be exported. Cause: {}".format(name, error),
        )
        return None
...
def __link_samplers(animation: gltf2_io.Animation, export_settings):
    ...
    for i, channel in enumerate(animation.channels):
        animation.channels[i].sampler = __append_unique_and_get_index(
            animation.samplers, channel.sampler
        )


def __get_blender_actions(
    ...
    if (
        ...
        if blender_object.data.shape_keys.animation_data.action is not None:
            ...
            blender_tracks[
                blender_object.data.shape_keys.animation_data.action.name
            ] = None
            action_on_type[
                blender_object.data.shape_keys.animation_data.action.name
            ] = "SHAPEKEY"
    ...
    return [
        (
            blender_action,
            blender_tracks[blender_action.name],
            action_on_type[blender_action.name],
        )
