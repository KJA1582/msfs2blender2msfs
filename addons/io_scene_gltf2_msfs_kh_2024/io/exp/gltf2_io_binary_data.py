...
class BinaryData:
    ...
    def from_list(
        ...
        if (
        ...
        ):  # Since Asobo uses a different data type for a certain component, we need to check if that's the case
            format_char = gltf2_io_constants.ComponentType.to_type_code_asobo(
                gltf_component_type
            )
            if format_char == "e":
            ...
            format_char = gltf2_io_constants.ComponentType.to_type_code(
                gltf_component_type
            )
