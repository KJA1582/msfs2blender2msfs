...
from io_scene_gltf2_msfs_kh_2024.io.com.gltf2_io import (
    from_dict,
    from_union,
    from_none,
    from_float,
...
from io_scene_gltf2_msfs_kh_2024.io.com.gltf2_io import to_float, to_class
...
class LightSpot:
    ...
    def from_dict(obj):
        ...
        inner_cone_angle = from_union(
            [from_float, from_none], obj.get("innerConeAngle")
        )
        outer_cone_angle = from_union(
            [from_float, from_none], obj.get("outerConeAngle")
        )
        return LightSpot(inner_cone_angle, outer_cone_angle)

    def to_dict(self):
        result = {}
        result["innerConeAngle"] = from_union(
            [from_float, from_none], self.inner_cone_angle
        )
        result["outerConeAngle"] = from_union(
            [from_float, from_none], self.outer_cone_angle
        )
        return result
...
class Light:
    ...
    def from_dict(obj):
        assert isinstance(obj, dict)
        color = from_union(
            [lambda x: from_list(from_float, x), from_none], obj.get("color")
        )
    ...
    def to_dict(self):
        ...
        result["color"] = from_union(
            [lambda x: from_list(to_float, x), from_none], self.color
        )
        result["intensity"] = from_union([from_float, from_none], self.intensity)
        result["spot"] = from_union(
            [lambda x: to_class(LightSpot, x), from_none], self.spot
        )
